...
class TestPartsManager(unittest.TestCase):
    def setUp(self):
        ...
        self.subpart1_1 = SimPart(
            sim=None, obj_name="P1_1", parent_obj=self.part1
        )
        self.subpart1_1_1 = SimPart(
            sim=None, obj_name="P1_1_1", parent_obj=self.subpart1_1
        )

    def test_walk_parts(self):
        part_mgr = self.part_mgr
        self.assertListEqual(
            part_mgr.top_level_parts(), [self.part1, self.part2]
        )
        self.assertListEqual(
            list(part_mgr.walk_parts()),
            [self.part1, self.subpart1_1, self.subpart1_1_1, self.part2],
        )
    ...
    def test_find_part_by_name(self):
        ...
        self.assertEqual(
            part_mgr.find_part_by_name("P1.P1_1"), self.subpart1_1
        )

        self.assertEqual(
            part_mgr.find_part_by_name("P1.P1_1.P1_1_1"), self.subpart1_1_1
        )
    ...
    def test_walk_ports(self):
        ...
        self.assertListEqual(
            list(part_mgr.walk_ports()),
            [in1, out1, io1.in_port(), io1.out_port()],
        )

        self.assertListEqual(
            list(part_mgr.walk_ports(SimOutputPort)), [out1, io1.out_port()]
        )
    ...
    def test_find_port_by_name(self):
        ...
        self.assertEqual(
            part_mgr.find_port_by_name("P1.P1_1.P1_1_1.Out1"), out1
        )
        self.assertEqual(part_mgr.find_port_by_name("P2.IO1"), io1)
        print(io1.in_port().hierarchy_name())
        self.assertEqual(
            part_mgr.find_port_by_name("P2.IO1_in"), io1.in_port()
        )
