...
class VtInPort(SimInputPort):
    ...
    * provides an API to read the messages from the buffer
    ...
    def __init__(self, sim, name, vThread, qDepth):
        ...
        Constructor
        '''
        # no msgReceived function, because msg_event() is overwritten in subclasses
        super().__init__(sim, vThread, name, msg_received_func=None)
...
class VtIOPort(SimIOPort):
    ...
    def __init__(self, sim, name, vThread, inPort):
        super().__init__(sim, vThread, name, msg_received_func=None, special_in_port=inPort)

    def readMsg(self, default=None):
        return self._in_port.readMsg(default)

    def nMsg(self):
        return self._in_port.nMsg()
    ...
    * the simPart timers are indirectly available to vThreads via vtTimers (set a flag on timeout)
    ...
    def __init__(self, sim, obj_name, parent_obj=None, remote_controlled=False,
        ...
        if remote_controlled:
            self.create_ports('in', ['threadControlPort'])

    def startThread(self):
        if self.thread is not None and self.thread.is_alive():
            raise RuntimeError(
                "startThread: old vThread %s still running" % self.obj_name())
    ...
    def waitUntilThreadTerminated(self):
        ...
        if self.thread is not None and self.thread.is_alive():
            ...
            if self.thread.is_alive():
                raise RuntimeError(
                    "waitUntilThreadTerminated: Thread %s did not terminate" % self.obj_name())
    ...
    def run(self):
        ...
        try:
            ...
            if termReason != "Terminated":
                self.annotation('vThread %s' % termReason)
    ...
    def runVThread(self):
        ...
        if self.target is not None:
            ...
            raise RuntimeError(
                "%s: No implementation for runVThread available\n" % self)
    ...
    def create_ports(self, ptype, listPortNames):
        ...
        if ptype == 'SamplingIn':
            ...
            SimPart.create_ports(self, ptype, listPortNames)
    ...
    def createElements(self, elems):
        ...
        for elType, names in elems.items():
            ...
            if elType == 'vtTmr':
                ...
                self.create_ports(elType, names)
