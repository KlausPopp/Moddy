...
def ethFlightTime(netSpeed, nBytes):
    """
    Compute flight time of an Ethernet frame
    If nBytes are below the minimum Ethernet frame length, the flight time of the minimum frame length
    is returned
...
def ethPdu( src, dst, ethType, payload):
    ...
    :return: filled Pdu
    """
    return Pdu( 'Eth', { 'src': src, 'dst': dst, 'type': ethType, 'payld': payload}, ethHdrLen())
