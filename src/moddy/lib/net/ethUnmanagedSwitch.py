...
class EthUnmanagedSwitch(simPart):
    def __init__(self, sim, obj_name, numPorts, netSpeed):
        ...
        self._num_ports = numPorts
        self._net_speed = netSpeed
    ...
    class NetPort:
        def __init__(self, switch, portNum):
            ...
            self._cutThroughDelPort = switch.newIOPort(
                "CutThroughDelPort%d" % portNum, self.cutThroughDelPortRecv
            )
            self._cutThroughDelPort.loopBind()

        def netPortRecvStart(self, inPort, pdu, outPort, flightTime):
            self._cutThroughDelPort.send(
                pdu, ethFlightTime(self._net_speed, ethHdrLen())
            )
        ...
        def sendPdu(self, pdu):
            self._netPort.send(
                pdu, ethFlightTime(self._net_speed, pdu.byteLen())
            )
